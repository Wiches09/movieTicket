...
func (h *UserHandler) SaveProfile(c echo.Context) error {
	...
	// Save to MongoDB
	err := h.repo.UpsertProfile(ctx, profile)
	if err != nil {
...
func (h *UserHandler) GetAllProfiles(c echo.Context) error {
	profiles, err := h.repo.GetAllProfiles(c.Request().Context())
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to fetch profiles"})
	}
